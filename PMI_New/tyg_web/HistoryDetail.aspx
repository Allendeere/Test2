<%@ Page Language="C#" AutoEventWireup="true" CodeFile="HistoryDetail.aspx.cs" Inherits="HistoryDetail" %>
<!DOCTYPE html>
...
<script type="text/javascript">
    var machineID = "<% Response.Write(Request.QueryString["m"].ToString()); %>";
    var orderID = "<% Response.Write(Request.QueryString["o"].ToString()); %>";
    var pg = 1;

    function soap(){
        tyg.setHead("<% Response.Write(Session["user"]); %>", true);
        mdetail.getList(machineID, orderID, pg);
    }

    function setDetailPage(target) {
        mdetail.getList(machineID, orderID, target);
    }
</script>
</head>
<body onload="soap()">
			...
			<div class="v1">
				<div class="ma_0_10">
					<h1 class="clearfix ma_10_0"><span class="fn_20 flo_l fn_black fn_bold main-title">歷史資料</span>
                        <a href="javascript:history.back();" class="btn btn-default btn-lg flo_r">回上一頁</a>
					</h1>
