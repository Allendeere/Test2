<%@ Page Language="C#" AutoEventWireup="true" CodeFile="PileSet_New.aspx.cs" Inherits="PileSet_New" %>
<!DOCTYPE html>
...
<script type="text/javascript">
    function soap() {
        ...
        hole.getFactoryOpion();
    }

    function getAreaOption(factoryID) {
        hole.getAreaOption(factoryID);
    }

    function getPlatformOption(areaID) {
        var factoryID = $("#factoryID").val();
        hole.getPlatformOption(factoryID, areaID);
    }

    function getMHoleList(platformID) {
        hole.getMHoleList(platformID);
    }
    ...
    function modMHole() {
        ...
        var MHoleID = $("#MHoleID").val();
        var MHoleSerial1 = $("#serial1").val();
        var MHoleSerial2 = $("#serial2").val();
        var MHoleSerial3 = $("#serial3").val();
        ...
        for (var checkbox of markedCheckbox) {
            ...
            //var foo = document.getElementById("bonju");
            //foo.appendChild(br);
            //foo.appendChild(result);
            hole.modMHole(setMode, factoryID, areaID, platformID, checkbox.value, MHoleSerial1, MHoleSerial2, MHoleSerial3, MRatio, updater);
        }
		//if (MHoleSerial1 != "") {
		//	if(MHoleSerial2 != "" ||MHoleSerial3 != ""){
		//		if(MRatio != ""){
		//			hole.modBatchMHole(setMode, factoryID, areaID, platformID, MHoleID, MHoleSerial1, MHoleSerial2, MHoleSerial3, MRatio, updater);
		//		}else{
					...
					<div id="form-bg" class="pd1 pl1 pr1">
						<form id="mat_station_edit" name="mat_station_edit" method="post" enctype="multipart/form-data" action="###">
							...
							<div class="row">
								...
								<div class="col-sm-8">
									<select class="w100" id="platformID" name="platformID" onchange="getMHoleList(this.value)"></select>									
								</div>
                                <div class="col-sm-2">
                                </div>
							</div>
							<div class="row">
								...
								<div class="col-sm-8">
									<select class="w100" id="MHole" name="MHole"></select>
								</div>
                                <div class="col-sm-2">
                                </div>
							</div>
							...
							<div id="MHoleList" class="ma_0_10">

							</div>
							<div class="row ">
								<div class="col-md-4 col-sm-4 col-xs-12 ma_btm_20">
									<input Type="button" value="料號查詢" onClick="hole.gotoList()" class="btn input_style3">
								</div>
								<div class="col-md-4 col-sm-4 col-xs-12 ma_btm_20">
									<input Type="button" value="取消" onClick="hole.gotoState()" class="btn input_style2">
								</div>
								<div class="col-md-4 col-sm-4 col-xs-12 ma_btm_20">
									<input type="button" value="儲存" onClick="modMHole()" class="btn input_style1">
								</div>
							</div>
						</form>
					</div>
