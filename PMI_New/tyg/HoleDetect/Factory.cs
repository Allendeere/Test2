...
namespace TYG.HoleDetect
{
    ...
    public class Factory : IDisposable
    {
        ...
        #endregion
        public List<string> platformIDD = new List<string>();//newee
        ...
        public void AddPlatformID(string platformID,string areaID) 
        { 
            this.platformIDD.Add($"{platformID}@{areaID}");
        }

        public Platform GetPlatformID(string platformID) //用PlatformID去撈Platform物件出來
        {
            Platform platform = null; 
            platformIDD.ForEach(x => {
                var xx = x.Split('@');
                if ( xx[0] == platformID)
                {
                    platform = GetArea(xx[1]).GetPlatform(xx[0]);
                }; 
            });
